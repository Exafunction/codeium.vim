...
function! codeium#Accept() abort
  ...
  " Delete all characters after the cursor until EOL + insert text
  let delete_eol = ""
  if col('.') < col('$')
    let delete_eol = "\<Esc>ld$a"
  endif
  let insert_text = "\<C-R>\<C-O>=codeium#CompletionText()\<CR>"

  " Exclude the completion prompt from the text
  let s:completion_text = strpart(text, col('.') - 1)
  ...
  return delete_eol . insert_text . cursor_text
endfunction
